...
std::string ConfigReader::read(const std::string& key, const std::string& defaultValue) const
{
    ...
    while ((pos = value.find("${")) != std::string::npos) {
        ...
        value = value.substr(0, pos) + read(key, "") + value.substr(endPos + 1);
    }
    return value;
}

unsigned long ConfigReader::readLong(const std::string& key, unsigned long defaultValue) const
{
...
double ConfigReader::readDouble(const std::string& key, double defaultValue) const
{
...
bool ConfigReader::readBool(const std::string& key, bool defaultBoolValue) const
{
